...
void ArkWebInnerConfigureNetworkContextParamsBefore(
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
          ::switches::kEnableNwebEx)) {
    network_context_params->is_strict_log_mode = context->IsOffTheRecord();
  }
#endif
}
...
void ArkWebInnerCreateThrottlesForNavigation(
  ...
  registry.AddThrottle(
      ohos_safe_browsing::UrlTrustListNavigationThrottle::Create(registry));
#endif
}
...
#if BUILDFLAG(ARKWEB_NETWORK_LOAD)
bool ChromeContentBrowserClientCef::SupportsAvoidUnnecessaryBeforeUnloadCheckSync() {
  return false;
}
