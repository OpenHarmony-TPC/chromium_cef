...
base::DictionaryValue VisitedUrlInfo::ToDict() {
  base::DictionaryValue dict;
  dict.SetString(kUrl, url());
  dict.SetInteger(kCount, count());
  dict.SetString(kOrigin, origin());
  return dict;
}
...
bool VisitedUrlInfo::operator==(const VisitedUrlInfo& info) {
  return url() == info.url();
}

// static
VisitedUrlInfo VisitedUrlInfo::FromDict(const base::DictionaryValue& dict) {
  std::string url;
  std::string origin;
  int count;
  dict.GetString(kUrl, &url);
  dict.GetString(kOrigin, &origin);
  dict.GetInteger(kCount, &count);
...
void PredictorDatabase::RecordVisitedUrl(VisitedUrlInfo url_info) {
  ...
  ListPrefUpdate update(pref_service, kVisitedUrls);
  if (is_update) {
...
void PredictorDatabase::UpdateFromPrefsAndClear() {
  ...
  if (last_visited_urls_.size() == 0 && last_visited_urls_sorted_.size() == 0) {
    ...
    auto last_visited_urls = pref_service->Get(kVisitedUrls);
    for (auto it = last_visited_urls->GetList().begin();
         it != last_visited_urls->GetList().end(); ++it) {
      last_visited_urls_.push_back(
          VisitedUrlInfo::FromDict(base::Value::AsDictionaryValue(*it)));
