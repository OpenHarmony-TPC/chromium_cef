...
std::string GetFullHash(const GURL& gurl) {
  ...
  return base::ToLowerASCII(base::HexEncode(full_hash.data(), full_hash.size()));
}
...
struct OhosSafeBrowsingTabHelper::SBCheck {
...
OhosSafeBrowsingTabHelper::SBCheck::SBCheck(
    const GURL& url,
    std::unique_ptr<network::SimpleURLLoader> simple_url_loader)
    : url(url),
...
OhosSafeBrowsingTabHelper::SBCheck::~SBCheck() = default;

OhosSafeBrowsingTabHelper::OhosSafeBrowsingTabHelper(content::WebContents* web_contents,
                                                     bool incognito_mode,
                                                     CefBrowserHostBase* browser)
    : content::WebContentsObserver(web_contents),
      content::WebContentsUserData<OhosSafeBrowsingTabHelper>(*web_contents) {
  ...
  sb_client_.reset(new OhosSbClient(web_contents, pref_service, incognito_mode));
  browser_ = browser;
}

OhosSafeBrowsingTabHelper::~OhosSafeBrowsingTabHelper() = default;

void OhosSafeBrowsingTabHelper::DidRedirectNavigation(
...
void OhosSafeBrowsingTabHelper::DidStartNavigation(
    content::NavigationHandle* navigation_handle) {
  if (!navigation_handle->IsInMainFrame()) {
    return;
  }
...
void OhosSafeBrowsingTabHelper::QuerySafeBrowsingResults(
    content::NavigationHandle* navigation_handle) {
  ...
  bool is_safe_browsing_enabled = browser_->IsSafeBrowsingEnabled();

  std::string package_name = "default";
  const base::CommandLine& command_line = 
      *base::CommandLine::ForCurrentProcess();
  if (command_line.HasSwitch(switches::kBundleName)) {
    package_name = base::CommandLine::ForCurrentProcess()->
  ...
  if (is_safe_browsing_enabled) {
    request_data.Set(kUrl, ConvertUrlQuery(navigation_handle->GetURL().spec()));
  }
  request_data.Set(kDomain, (navigation_handle->GetURL().scheme() + "://" + navigation_handle->GetURL().host()));
  ...
  std::string converted_url = is_safe_browsing_enabled
  ...
  loader->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
      ...
      base::BindOnce(&OhosSafeBrowsingTabHelper::OnSimpleLoaderComplete,
                     base::Unretained(this), it, navigation_handle->GetURL(), converted_url));
}

void OhosSafeBrowsingTabHelper::OnSimpleLoaderComplete(
  ...
  if (hw_code >= 0 && (policy >= OHSBPolicyType::POLICY_NO_PROMPT || policy <= OHSBPolicyType::POLICY_NO_PROMPT)) {
  ...
  for (auto& check_result : response.hash_check_result) {
    for (auto& hash_result : check_result.hash_result_list) {
      if (hash_result.url_hash == GetFullHash(url)) {
        ...
        LOG(INFO) << "SafeBrowsing hit url check result policy: " << hash_policy << ", url_hw_code: " << hash_hw_code;
        if (hash_hw_code < 0 ||
...
void OhosSafeBrowsingTabHelper::DidFinishNavigation(
    content::NavigationHandle* navigation_handle) {}

void OhosSafeBrowsingTabHelper::NavigationStopped() {}

void OhosSafeBrowsingTabHelper::OnConnectionChanged(
    network::mojom::ConnectionType type) {}

WEB_CONTENTS_USER_DATA_KEY_IMPL(OhosSafeBrowsingTabHelper);
