...
void CefBrowserPlatformDelegateAlloy::SetAccessibilityState(
  ...
  if (accessibility_state == STATE_ENABLED) {
    ...
    if (!scoped_accessibility_enabled_) {
      scoped_accessibility_enabled_ = true;
      scoped_accessibility_mode_ =
          content::BrowserAccessibilityState::GetInstance()
              ->CreateScopedModeForProcess(accMode);
    }
  } else if (accessibility_state != STATE_ENABLED && scoped_accessibility_enabled_) {
    scoped_accessibility_enabled_ = false;
