...
OhGinJavascriptBridgeDispatcher::GetH5ObjectMethodNames(
  ...
  if (try_catch.HasCaught() || !maybe_value.ToLocal(&value)) {
    ...
    for (uint32_t i = 0; i < keys->Length(); ++i) {
      ...
      int len = 0;
      if (key->IsString()) {
        len = key.As<v8::String>()->Utf8LengthV2(isolate);
      } else {
        LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::GetH5ObjectMethodNames "
                      "key is not a string";
        continue;
      ...
      key.As<v8::String>()->WriteUtf8V2(isolate, buf, len, v8::String::WriteFlags::kReplaceInvalidUtf8);
...
void OhGinJavascriptBridgeDispatcher::DoCallH5Function(
    int32_t frame_routing_id,
    int32_t h5_object_id,
    const std::string& method_name,
    base::Value::List args) {
  OnDoCallH5Function(frame_routing_id, h5_object_id, method_name, args);
}

void OhGinJavascriptBridgeDispatcher::RemoveNamedObject(const std::string& name) {
  // Removal becomes in effect on next reload. We simply removing the entry
  // from the map here.
...
void OhGinJavascriptBridgeDispatcher::SetHost(
    mojo::PendingRemote<content::mojom::OhGinJavascriptBridgeHost> host) {
  CHECK(!remote_);
  CHECK(named_objects_.empty());
  remote_.Bind(std::move(host));
}

content::mojom::OhGinJavascriptBridgeHost* OhGinJavascriptBridgeDispatcher::GetRemoteObjectHost() {
