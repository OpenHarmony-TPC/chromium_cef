...
const int kMaxContextImageNodeSize = 1024;
...
bool NeedsDownscale(const gfx::Size& original_image_size) {
  if (original_image_size.width() <= kMaxContextImageNodeSize &&
      original_image_size.height() <= kMaxContextImageNodeSize)
    return false;
  return true;
}
...
SkBitmap Downscale(const SkBitmap& image) {
  ...
  if (scaled_size.width() > kMaxContextImageNodeSize) {
    scaled_size.Scale(kMaxContextImageNodeSize / scaled_size.width());
  }

  if (scaled_size.height() > kMaxContextImageNodeSize) {
    scaled_size.Scale(kMaxContextImageNodeSize / scaled_size.height());
  }
...
void CefFrameImpl::GetImageForContextNode() {
  ...
  SkBitmap resize_image = Downscale(image);
