...
class CefDistillCallback : public virtual CefBaseRefCounted {
  public:
  virtual void OnDistillCallback(uint64_t request_id, const std::string& distill_info) = 0;
};
#endif // ARKWEB_READER_MODE
...
class ArkWebBrowserHostExt : public virtual CefBrowserHost,
 ...
 public:
  ...
  /// GetAllImage
  ///
  virtual void GetAllImage(int32_t taskid, const std::vector<std::string>& url) {}

  ///
  /// GetAllImageByXPath
  ///
  virtual void GetAllImageByXPath(int32_t taskid, const std::vector<std::string>& xpath) {}
  ...
  ///
  /// is ElementExist by xpath.
  ///
  virtual bool IsElementExist(CefString& xPath) = 0;
...
#if BUILDFLAG(ARKWEB_READER_MODE)
  virtual void Distill(uint64_t request_id, const DistillOptions& distill_options,
