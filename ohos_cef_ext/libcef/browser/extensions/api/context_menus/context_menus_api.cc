...
#include "libcef/browser/extensions/api/context_menus/context_menus_api_helpers.h"
...
ExtensionFunction::ResponseAction ContextMenusCreateFunction::Run() {
  ...
  if (params->create_properties.id) {
    ...
    std::optional<int> result = properties.GetDict().FindInt(
        extensions::cef::context_menus_api_helpers::kGeneratedIdKey);
    EXTENSION_FUNCTION_VALIDATE(result);
    id.uid = *result;
  }
 
  std::string error;
  if (!extensions::cef::context_menus_api_helpers::CreateMenuItem(
...
ExtensionFunction::ResponseAction ContextMenusUpdateFunction::Run() {
  ...
  std::string error;
  if (!extensions::cef::context_menus_api_helpers::UpdateMenuItem(
...
ExtensionFunction::ResponseAction ContextMenusRemoveFunction::Run() {
  ...
  if (!item || item->extension_id() != extension_id()) {
    return RespondNow(
        Error(extensions::cef::context_menus_api_helpers::kCannotFindItemError,
              extensions::cef::context_menus_api_helpers::GetIDString(id)));
  }
 
  if (!manager->RemoveContextMenuItem(id))
    return RespondNow(Error("Cannot remove menu item."));
  CefMenuManager::OnContextMenusRemove(extension_id(), *params->menu_item_id.as_string);
...
ExtensionFunction::ResponseAction ContextMenusRemoveAllFunction::Run() {
  ...
  CefMenuManager::OnContextMenusRemoveAll(extension_id());
