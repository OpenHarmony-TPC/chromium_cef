...
namespace {
blink::WebHitTestResult HitTestResultForTap(blink::WebView* web_view,
                                            const gfx::Point& tap_point,
                                            const gfx::Size& tap_area) {
  if (!web_view) {
    return blink::HitTestResult();
  }

  blink::WebViewImpl* web_view_impl = blink::DynamicTo<blink::WebViewImpl>(web_view);
  if (!web_view_impl || !web_view_impl->GetPage()) {
    return blink::HitTestResult();
  }

  auto* main_frame =
      blink::DynamicTo<blink::LocalFrame>(web_view_impl->GetPage()->MainFrame());
  if (!main_frame) {
    return blink::HitTestResult();
  }
  blink::WebLocalFrameImpl* local_frame_impl =
      blink::WebLocalFrameImpl::FromFrame(main_frame);
  if (!local_frame_impl) {
    return blink::HitTestResult();
  }

  blink::WebGestureEvent tap_event(blink::WebInputEvent::Type::kGestureTap,
  ...
  blink::WebGestureEvent scaled_event =
      blink::TransformWebGestureEvent(local_frame_impl->GetFrameView(), tap_event);

  blink::HitTestResult result =
#if BUILDFLAG(ARKWEB_MENU)
      main_frame->GetEventHandler()
          .HitTestResultForGestureEvent(scaled_event,
                                        blink::HitTestRequest::kReadOnly |
                                            blink::HitTestRequest::kActive |
                                            blink::HitTestRequest::kOnDoHitTest)
          .GetHitTestResult();
#else
      main_frame->GetEventHandler()
          .HitTestResultForGestureEvent(
              scaled_event, blink::HitTestRequest::kReadOnly | blink::HitTestRequest::kActive)
          .GetHitTestResult();
...
int GetSystemTotalMem() {
  ...
  if (base::GetSystemMemoryInfo(&meminfo)) {
    return static_cast<int>(meminfo.total.InBytes());
  }
...
GURL ArkwebFrameExtImpl::GetAbsoluteUrl(const blink::WebNode& node,
  ...
  return GURL(node.GetDocument().CompleteURL(
      blink::WebString::FromUTF16(url_fragment)).GetString().Utf8());
}
...
void ArkwebFrameExtImpl::SendHitEvent(cef::mojom::HitEventParamsPtr params) {
  ...
  // TODO(arkweb): The HitTestResultForTap interface is added.
  const blink::WebHitTestResult result =
      HitTestResultForTap(webview, gfx::Point(params->x, params->y),
                                   gfx::Size(params->width, params->height));
  cef::mojom::HitDataParamsPtr data = cef::mojom::HitDataParams::New();
  GURL absolute_image_url = GURL(result.AbsoluteImageURL().GetString().Utf8());
  ...
  PopulateHitTestData(GURL(result.AbsoluteLinkURL().GetString().Utf8()), absolute_image_url,
...
void ArkwebFrameExtImpl::SetFocusByPosition(
  ...
  // TODO(arkweb): The HitTestResultForTap interface is added.
  const blink::WebHitTestResult result =
      HitTestResultForTap(webview, gfx::Point(x, y), gfx::Size(1, 1));
