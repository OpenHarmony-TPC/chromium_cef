...
void OhGinJavascriptBridgeDispatcherHost::AddNamedObject(
  ...
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  // 为了兼容老版本webcotroller方式,
  // 目前webcontroller把object_id保存在core侧,以object_name去ace侧获取对象
  // 新的webviewcontroller方式,把object_id保存在web_webviwe侧,以object_id去web_webviwe侧获取对象
  if (object_id ==
