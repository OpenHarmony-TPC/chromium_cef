 ...
 public:
  UnhandledRequest(const std::vector<PermissionType> permissions,
      ...
      : permissions_(permissions),
        ...
        results_(permissions.size(), content::PermissionResult(blink::mojom::PermissionStatus::DENIED)),
        cancelled_(false) {
  ...
  void SetPermissionStatus(PermissionType type, PermissionStatus status) {
    ...
    DCHECK(!IsCompleted());
    results_[result->second] = content::PermissionResult(status);
...
PermissionStatus AlloyPermissionManager::GetPermissionStatus(
  ...
  } else if (permission_type == PermissionType::LOCAL_NETWORK_ACCESS) {
    // Follow AW, LNA permission auto-granted.
    return PermissionStatus::GRANTED;
