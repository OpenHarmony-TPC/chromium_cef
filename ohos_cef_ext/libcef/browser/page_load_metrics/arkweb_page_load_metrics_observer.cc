...
#if BUILDFLAG(ARKWEB_SAFEBROWSING)
  if (web_contents) {
    GURL lastCommittedURL = web_contents->GetLastCommittedURL();
    ohos_safe_browsing::SafeBrowsingSnapshotManager::GetInstance().SetFMP(
        lastCommittedURL.spec(), web_contents->GetNWebId());
  }
#endif
...
#if defined(REPORT_SYS_EVENT) && BUILDFLAG(ARKWEB_NETWORK_DFX)
  web_performance_timing_.first_contentful_paint = first_contentful_paint_ms;
#endif
  int64_t navigation_start_time = GetNavigationStartTime();
  ReportFirstContentfulPaint(navigation_start_time, first_contentful_paint_ms);
