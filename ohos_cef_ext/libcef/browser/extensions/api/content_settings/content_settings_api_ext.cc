...
{
    static const base::NoDestructor<std::map<ChromeSettingScope, std::string>> scope_map({
        {ChromeSettingScope::kNone, "None"},
...
// NWebExtensionContentSettingsGetParamRelease to free any partially allocated memory
// in getParam
...
ExtensionFunction::ResponseAction ContentSettingsContentSettingGetFunction::Run() 
{
    ...
    EXTENSION_FUNCTION_VALIDATE(RemoveContentType(GetMutableArgs(), &content_type));
    ...
    const base::WeakPtr<ContentSettingsContentSettingSetFunction>& function, const char* error)
{
    ...
    if (error) {
        ...
        LOG(INFO) << "ContentSettingsContentSetting SetCallback no error";
