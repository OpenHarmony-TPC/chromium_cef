...
// #include "third_party/blink/renderer/platform/widget/input/input_handler_proxy.h"  // Removed for Chromium 141 compatibility
...
class CefGestureEventCallbackImpl : public CefGestureEventCallback {
 public:
  using CallbackType = GestureEventCallback;
...
class CefMouseEventCallbackImpl : public CefMouseEventCallback {
 public:
  using CallbackType = MouseEventCallback;
...
gfx::Size ArkWebRenderWidgetHostViewOSRExt::GetVisibleViewportSizeDevicePx() {
  gfx::Size bounds = GetPhysicalVisibleViewportSize();
#if BUILDFLAG(ARKWEB_EXT_TOPCONTROLS)
  bounds.set_height(bounds.height() - GetShrinkViewportHeight());
#endif
  return gfx::ScaleToCeiledSize(bounds, GetDeviceScaleFactor());
}
#endif
    ...
    switch (web_event.GetType()) {
      ...
      case blink::WebInputEvent::Type::kGestureTap:
        selection_controller_->HandleTapEvent(
            web_event.PositionInWidget(), web_event.TapCount());
...
#endif //BUILDFLAG(ARKWEB_EXT_PULL_TO_REFRESH) || BUILDFLAG(ARKWEB_PULL_TO_REFRESH)
}
...
void ArkWebRenderWidgetHostViewOSRExt::OnUpdateTextInputStateCalled(
  ...
  if (state && state->type != ui::TEXT_INPUT_TYPE_NONE) {
    // The last definition of the enumeration in src/cef/include/internal/cef_types.h was modified to 
    // clearly indicate "the number of values," making the semantics clearer. However, the corresponding 
    // enumeration in src/ui/base/ime/text_input_mode.h has not been modified, 
    // so it is necessary to subtract one to match the difference.
    static_assert(
        static_cast<int>(ArkWebRenderHandlerExt::TextInputMode::CEF_TEXT_INPUT_MODE_NUM_VALUES) - 1 ==
...
std::string ArkWebRenderWidgetHostViewOSRExt::GetCurrentLanguage() {
  if (language_== std::string("und")) {
    LOG(WARNING) << "[CefRenderWidgetHostViewOSR::GetCurrentLanguage] Current language identifier"
                 << " is 'Undetermined (und)', triggering retry mechanism";
    if (render_widget_host() && render_widget_host()->AsRenderWidgetHostImplExt())
      render_widget_host()->AsRenderWidgetHostImplExt()->ReSendCurrentLanguage();
    else {
      LOG(WARNING) << "render_widget_host() is null";
    }
  }
  LOG(INFO) << "[CefRenderWidgetHostViewOSR::GetCurrentLanguage] Returning final language identifier: " << language_;
  return language_;
}
#endif
...
void ArkWebRenderWidgetHostViewOSRExt::ModifyRenderFit(int32_t fitType) {
  if (!browser_impl_ || !browser_impl_->client()) {
    LOG(ERROR) << "ModifyRenderFit get client failed.";
    return;
  }
  CefRefPtr<CefRenderHandler> handler =
      browser_impl_->client()->GetRenderHandler();
  if (handler && handler->AsArkWebRenderHandler()) {
    handler->AsArkWebRenderHandler()->ModifyRenderFit(fitType);
  }
}
#endif