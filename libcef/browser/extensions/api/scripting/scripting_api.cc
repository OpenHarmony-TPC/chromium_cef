...
#include "cef/libcef/browser/extensions/api/scripting/scripting_api.h"
...
ScriptExecutor* GetScriptExecutor(int tab_id, std::string* error_out) {
#ifdef OHOS_ARKWEB_EXTENSIONS
  CefRefPtr<AlloyBrowserHostImpl> browser =
      GetClientAvailableBrowser(tab_id);
  if (!browser || (browser->GetTabId() != tab_id)) {
    LOG(ERROR) << "No tab with id: " << tab_id;
    *error_out = base::StringPrintf("No tab with id: %d", tab_id);
    return nullptr;
  }

  ScriptExecutor* script_executor =
      CefExtensionWebContentsObserver::FromWebContents(
             browser->web_contents())
      ->script_executor();
  if (!script_executor) {
    LOG(ERROR) << "No script_executor with id: " << tab_id;
    *error_out = base::StringPrintf("No script_executor with id: %d",
                                    tab_id);
    return nullptr;
  }
  return script_executor;
#else
  return tab_helper->script_executor();
#endif // OHOS_ARKWEB_EXTENSIONS
}
...
bool CanAccessTarget(const PermissionsData& permissions,
                     ...
                     std::string* error_out) {
  content::WebContents* tab = GetWebContents(target.tab_id);
  if (!tab) {
  ...
  ScriptExecutor* script_executor = GetScriptExecutor(target.tab_id, error_out);
