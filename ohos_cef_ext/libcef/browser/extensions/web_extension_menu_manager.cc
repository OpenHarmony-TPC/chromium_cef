...
void ExtensionContextMenusInvokeActiveTab(
  ...
  if (extensions::ExtensionTabUtil::GetTabById(tab_id, context, true, &out_contents)) {
    auto* active_tab_granter = extensions::ActiveTabPermissionGranter::FromWebContents(out_contents);
    if (active_tab_granter) {
      active_tab_granter->GrantIfRequested(extension);
    }
  }
}
...
CefWebExtensionMenuManager::GetAllExtensionContextMenus(  const std::vector<std::string>& extension_ids) {
  ...
  extensions::MenuManager* menu_manager =
      extensions::MenuManager::Get(browser_context);  if (!menu_manager) {
    LOG(ERROR) << "menu_manager is null";
    return items;
  }
