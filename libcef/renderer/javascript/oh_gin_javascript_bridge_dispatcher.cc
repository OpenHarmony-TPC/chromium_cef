...
std::vector<std::string> OhGinJavascriptBridgeDispatcher::GetH5ObjectMethodNames(
  ...
  for (uint32_t i = 0; i < keys->Length(); ++i) {
    ...
    value = object->Get(context, key).ToLocalChecked();
    if(!value->IsFunction()) {
      continue;
    }
...
bool OhGinJavascriptBridgeDispatcher::HasH5ObjectMethod(
  ...
  if(!object->GetPropertyNames(context).ToLocal(&keys)) {
    return false;
  }
  ...
  for (uint32_t i = 0; i < keys->Length(); ++i) {
    ...
    v8::Maybe<bool> has_maybe = object->HasOwnProperty(context, key.As<v8::String>());
    bool has_own_property = has_maybe.FromMaybe(false);
    if(!has_own_property) {
      continue;
    }
