...
void AlloyBrowserHostImplExt::UpdatePinnedExtensionIds(
  ...
  if (isPinned) {
    ...
    auto iter = std::find(storedPinnedIds.begin(), storedPinnedIds.end(), extensionId);
...
void AlloyBrowserHostImplExt::ShowPopupMenu(
  ...
  if (platform_delegate_) {
    platform_delegate_->AsArkWebCefBrowserPlatformDelegateExt()->ShowPopupMenu(
...
void AlloyBrowserHostImplExt::ReportWindowStatus(bool first_view_ready) {
  ...
  if (auto render_view_host = contents->GetRenderViewHost()) {
    ...
    int rph_unique_id = render_process_host->GetID().value();
    if (process.IsValid()) {
