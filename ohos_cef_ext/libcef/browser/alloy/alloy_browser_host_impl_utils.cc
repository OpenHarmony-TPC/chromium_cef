...
int AlloyBrowserHostImplUtils::handleZoomEventWithInput(bool zoom_in) {
  ...
  if (base::CommandLine* command_line = base::CommandLine::ForCurrentProcess())
    usePreset = command_line->HasSwitch(switches::kEnableNwebEx);
...
void AlloyBrowserHostImplUtils::EvictFrameBackBuffersWhenNWebWasHidden() {
  if (alloyBrowserHostImpl == nullptr) {
    LOG(WARNING) << "alloyBrowserHostImpl is nullptr";
    return;
  }
