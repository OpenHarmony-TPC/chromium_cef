...
void AlloyScreenCaptureAccessRequest::ReportRequestResult(bool allowed) {
  ...
  if (request_.requested_video_device_ids.empty() ||
    ...
    LOG(WARNING) << "[screen_webrtc_logging]ReportRequestResult, requested_video_device_id is not empty";
    media_id = content::DesktopMediaID::Parse(
        request_.requested_video_device_ids.front());
  }
  if (media_id.is_null()) {
    LOG(ERROR) << "[screen_webrtc_logging]ReportRequestResult, media_id is invalid(type == TYPE_NONE)";
