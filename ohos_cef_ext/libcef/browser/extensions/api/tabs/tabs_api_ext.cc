...
void TabsMoveFunction::OnTabMoved(
  ...
  if (error) {
    ...
    WebContents* contents = nullptr;
    if (!ExtensionTabUtil::GetTabById(tabs[0].id.value_or(-1), function->browser_context(),
        function->include_incognito_information(), &contents) || !contents) {
    ...
    for (NWebExtensionTab tab : tabs) {
      WebContents* contents = nullptr;
      if (!ExtensionTabUtil::GetTabById(tab.id.value_or(-1), function->browser_context(),
          function->include_incognito_information(), &contents) || !contents) {
...
ExtensionFunction::ResponseAction TabsQueryFunction::Run() {
  ...
  for (NWebExtensionTab tab : tabs) {
    ...
    if (!ExtensionTabUtil::GetTabById(
        ...
        include_incognito_information(), &contents) || !contents) {
      LOG(INFO) << "TabsQueryFunction cannot find WebContents by tab_id:" << tab.id.value_or(-1);
      return RespondNow(Error("TabsQueryFunction cannot find WebContents"));
    }
    ExtensionTabUtil::ScrubTabBehavior scrub_tab_behavior =
        ExtensionTabUtil::GetScrubTabBehavior(extension(), source_context_type(), contents);
    scrub_tab_behaviors.emplace_back(scrub_tab_behavior);
...
ExtensionFunction::ResponseAction TabsGetSelectedFunction::Run() {
  ...
  if (!ExtensionTabUtil::GetTabById(
      ...
      include_incognito_information(), &contents) || !contents) {
    LOG(INFO) << "TabsGetSelectedFunction cannot find WebContents by tab_id:" << tabs[0].id.value_or(-1);
    return RespondNow(Error("TabsGetSelectedFunction cannot find WebContents"));
  }
  ExtensionTabUtil::ScrubTabBehavior scrub_tab_behavior =
      ExtensionTabUtil::GetScrubTabBehavior(extension(), source_context_type(), contents);
