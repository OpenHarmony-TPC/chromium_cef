...
void OhGinJavascriptBridgeDispatcherHost::ClearAllReceivers() {
...
content::mojom::OhGinJavascriptBridge* OhGinJavascriptBridgeDispatcherHost::GetJavaBridge(
  ...
  if (it == remotes_.end()) {
    ...
    bound_remote.set_disconnect_handler(
        base::BindOnce(&OhGinJavascriptBridgeDispatcherHost::RemoteDisconnected,
                       base::Unretained(this), routing_id));

    mojo::PendingReceiver<content::mojom::OhGinJavascriptBridgeHost> host_receiver;
    bound_remote->SetHost(host_receiver.InitWithNewPipeAndPassRemote());
    
    receivers_.Add(this, std::move(host_receiver), routing_id);
...
void OhGinJavascriptBridgeDispatcherHost::RemoteDisconnected(
    const content::GlobalRenderFrameHostId& routing_id) {
  remotes_.erase(routing_id);

  auto* frame_host = content::RenderFrameHost::FromID(routing_id);
...
void OhGinJavascriptBridgeDispatcherHost::RenderFrameCreated(
  ...
  for (ObjectMethodMap::const_iterator iter = async_method_map_.begin();
    ...
    GetJavaBridge(render_frame_host, /*should_create=*/true)
        ->AddNamedObject(iter->second.first, iter->first, std::move(async_method_list), true);
  }
  for (const auto& [object_id, object_name_set] : object_id_map_) {
    for (const auto& object_name : object_name_set) {
      ...
      GetJavaBridge(render_frame_host, /*should_create=*/true)
          ->AddNamedObject(object_name, object_id, std::move(empty_list), false);
    }
  }
}
...
void OhGinJavascriptBridgeDispatcherHost::WebContentsDestroyed() {
  ...
  CEF_POST_TASK(
    CEF_UIT,
    base::BindOnce(&OhGinJavascriptBridgeDispatcherHost::ClearAllReceivers,
  ...
  web_contents()
    ...
    ->ForEachRenderFrameHostImplIncludingSpeculative(
        [&object_name, &async_method_for_render,
          ...
          GetJavaBridge(render_frame_host, /*should_create=*/true)
              ->AddNamedObject(object_name, this->object_id_, std::move(async_method_for_render),
                               true);
        });
}
      ...
      ->ForEachRenderFrameHostImplIncludingSpeculative(
          [&object_name, &async_method_for_render, object_id,
           this](content::RenderFrameHostImpl* render_frame_host) {
            ...
            GetJavaBridge(render_frame_host, /*should_create=*/true)
                ->AddNamedObject(object_name, object_id, std::move(async_method_for_render),
                                 true);
          });
}
...
void OhGinJavascriptBridgeDispatcherHost::RemoveNamedObjectInternal(
    const std::string& object_name,
    bool is_async) {
  LOG(DEBUG)
...
bool OhGinJavascriptBridgeDispatcherHost::CheckIsInJsPermission(
  ...
  if (map_tmp[object_id].find("") == map_tmp[object_id].end() ||
      map_tmp[object_id][""].find(gurl.GetHost()) ==
  ...
  JsProxyPermissionConfigData obj_permission =
      javascript_sync_permission_map_[object_id][""][gurl.GetHost()];
  ...
  if (map_tmp[object_id][method_name].find(gurl.GetHost()) ==
  ...
  JsProxyPermissionConfigData method_permission =
      javascript_sync_permission_map_[object_id][method_name][gurl.GetHost()];
...
void OhGinJavascriptBridgeDispatcherHost::OnInvokeMethod(
  ...
  if (!CheckIsInJsPermission(document_url, method_name, object_id, false) ||
    ...
    *error_code =
        content::mojom::OhGinJavascriptBridgeError::kOhGinJavascriptBridgePermissionDenied;
    return;
  }
  ...
  if (!client_) {
    LOG(ERROR) << "OhGinJavascriptBridgeDispatcherHost::OnInvokeMethod: "
                  "client_ is null";
    *error_code =
        content::mojom::OhGinJavascriptBridgeError::kOhGinJavascriptBridgeClientDeleted;
    return;
  }
  ...
  *error_code = static_cast<content::mojom::OhGinJavascriptBridgeError>(error);
  if (error != 0) {
    return;
  }
...
void OhGinJavascriptBridgeDispatcherHost::DoCallH5Function(
  ...
  if (target_frame) {
    ...
    GetJavaBridge(target_frame, /*should_create=*/true)
        ->DoCallH5Function(target_frame->GetRoutingID(), h5_object_id, h5_method_name, std::move(base_args));
  }
}

void OhGinJavascriptBridgeDispatcherHost::GetMethods(int32_t object_id, GetMethodsCallback callback) {
  std::set<std::string> method_names;
  OnGetMethods(object_id, &method_names);
  std::move(callback).Run(std::vector<std::string>(method_names.begin(), method_names.end()));
}

void OhGinJavascriptBridgeDispatcherHost::HasMethod(int32_t object_id,
                                                    const std::string& method_name,
                                                    HasMethodCallback callback) {
  bool result = false;
  OnHasMethod(object_id, method_name, &result);
  std::move(callback).Run(result);
}

void OhGinJavascriptBridgeDispatcherHost::InvokeMethod(const std::string& method_name,
                                                       const std::string& document_url,
                                                       base::Value::List arguments,
                                                       InvokeMethodCallback callback) {
  bool isAsyncthread = false;
  OnHasAsyncThreadMethod(object_receivers_.current_context().second, method_name, &isAsyncthread);
  if (isAsyncthread) {
    async_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&OhGinJavascriptBridgeDispatcherHost::InvokeMethodInternal,
                                  base::WrapRefCounted(this),
                                  object_receivers_.current_context().first.frame_routing_id,
                                  object_receivers_.current_context().second, method_name, document_url,
                                  std::move(arguments), std::move(callback)));
  } else {
    InvokeMethodInternal(object_receivers_.current_context().first.frame_routing_id,
                         object_receivers_.current_context().second, method_name, document_url,
                         std::move(arguments), std::move(callback));
  }
}

void OhGinJavascriptBridgeDispatcherHost::InvokeMethodInternal(int routing_id,
                                                               int32_t object_id,
                                                               const std::string& method_name,
                                                               const std::string& document_url,
                                                               base::Value::List arguments,
  ...
  base::Value::List wrapped_result;
  content::mojom::OhGinJavascriptBridgeError error_code
      = content::mojom::OhGinJavascriptBridgeError::kOhGinJavascriptBridgeNoError;
  OnInvokeMethod(routing_id, object_id, document_url, method_name,
                 std::move(arguments), &wrapped_result, &error_code);

  OnInvokeMethodCallback(error_code, std::move(wrapped_result), std::move(callback));
}

void OhGinJavascriptBridgeDispatcherHost::OnInvokeMethodCallback(content::mojom::OhGinJavascriptBridgeError error_code,
    base::Value::List wrapped_result,
    InvokeMethodCallback callback) {
  if (!CEF_CURRENTLY_ON_UIT()) {
    CEF_POST_TASK(CEF_UIT,
                  base::BindOnce(&OhGinJavascriptBridgeDispatcherHost::OnInvokeMethodCallback,
                                 base::WrapRefCounted(this), error_code,
                                 std::move(wrapped_result), std::move(callback)));
    return;
  }
...
void OhGinJavascriptBridgeDispatcherHost::InvokeMethodAsync(const std::string& method_name,
                                                            const std::string& document_url,
                                                            base::Value::List arguments) {
  LOG(INFO) << "adrainlyz0130 OhGinJavascriptBridgeDispatcherHost::InvokeMethodAsync document_url: " << document_url;
  OnInvokeMethodAsync(object_receivers_.current_context().first.frame_routing_id,
                 object_receivers_.current_context().second, document_url, method_name,
                 arguments);
}

void OhGinJavascriptBridgeDispatcherHost::InvokeMethodFlowbuf(const std::string& method_name,
                                                              const std::string& document_url,
                                                              base::Value::List arguments,
                                                              int fd,
  ...
  if (isAsyncthread) {
    async_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&OhGinJavascriptBridgeDispatcherHost::InvokeMethodFlowbufInternal,
                                  base::WrapRefCounted(this),
                                  object_receivers_.current_context().first.frame_routing_id,
                                  object_receivers_.current_context().second, method_name, document_url,
                                  std::move(arguments), fd, std::move(callback)));
  } else {
    InvokeMethodFlowbufInternal(object_receivers_.current_context().first.frame_routing_id,
                                object_receivers_.current_context().second, method_name, document_url,
                                std::move(arguments), fd, std::move(callback));
  }
}

void OhGinJavascriptBridgeDispatcherHost::InvokeMethodFlowbufInternal(int routing_id,
                                                                      int32_t object_id,
                                                                      const std::string& method_name,
                                                                      const std::string& document_url,
                                                                      base::Value::List arguments,
  ...
  content::mojom::OhGinJavascriptBridgeError error_code
      = content::mojom::OhGinJavascriptBridgeError::kOhGinJavascriptBridgeNoError;
  OnInvokeMethodFlowbuf(routing_id, object_id, document_url, method_name,
                        std::move(arguments), fd, &wrapped_result, &error_code);

  OnInvokeMethodFlowbufCallback(error_code, std::move(wrapped_result), std::move(callback));
}

void OhGinJavascriptBridgeDispatcherHost::OnInvokeMethodFlowbufCallback(
    content::mojom::OhGinJavascriptBridgeError error_code,
    base::Value::List wrapped_result,
    InvokeMethodFlowbufCallback callback) {
  if (!CEF_CURRENTLY_ON_UIT()) {
    CEF_POST_TASK(CEF_UIT,
                  base::BindOnce(&OhGinJavascriptBridgeDispatcherHost::OnInvokeMethodFlowbufCallback,
                                 base::WrapRefCounted(this), error_code,
                                 std::move(wrapped_result), std::move(callback)));
    return;
  }
...
void OhGinJavascriptBridgeDispatcherHost::GetObject(
    int32_t object_id,
    mojo::PendingReceiver<content::mojom::OhGinJavascriptBridgeRemoteObject> receiver) {
  if (object_receivers_.empty()) {
    object_receivers_.set_disconnect_handler(base::BindRepeating(
        &OhGinJavascriptBridgeDispatcherHost::ObjectDisconnected, this));
  }
...
void OhGinJavascriptBridgeDispatcherHost::ObjectDisconnected() {
  OnObjectWrapperDeleted(object_receivers_.current_context().first.frame_routing_id,
                         object_receivers_.current_context().second);
}

void OhGinJavascriptBridgeDispatcherHost::ObjectWrapperDeleted(int32_t object_id) {
  (void)object_id;
}
