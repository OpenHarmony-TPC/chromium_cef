...
#if BUILDFLAG(ARKWEB_NWEB_EX) || BUILDFLAG(ARKWEB_NETWORK_BASE)
...
void ArkWebDealWithPostData(const std::string& post_data, 
  ...
  if (post_data.empty()) {
      ...
      params->post_data = network::ResourceRequestBody::CreateFromBytes(
          std::vector<uint8_t>(post_data.begin(), post_data.end()));
  }
}

#if BUILDFLAG(ARKWEB_EXT_HTTPS_UPGRADES)
void ArkWebBrowserHostExtImpl::LoadUrlWithParams(
  ...
  if (!CEF_CURRENTLY_ON_UIT()) {
    CEF_POST_TASK(
        ...
        base::BindOnce(&ArkWebBrowserHostExtImpl::LoadUrlWithParams, this,
                       std::string(url), load_type, std::string(refer),
                       std::string(headers), std::string(post_data),
                       allow_https_upgrade, transition_type));
    return;
  }
...
void ArkWebBrowserHostExtImpl::UpdateLocale(const CefString& locale) {
  ...
  if (!ui::ResourceBundle::HasSharedInstance() ||
      !ui::ResourceBundle::LocaleDataPakExists(update_locale, ui::ResourceBundle::Gender::kDefault)) {
...
void ArkWebBrowserHostExtImpl::GetAllImage(int32_t taskid, const std::vector<std::string>& urls) {
  LOG(INFO) << "ArkWebBrowserHostExtImpl::GetAllImage";
  auto web_contents = GetWebContents();
  auto frame = GetMainFrame();
  if (web_contents && frame && frame->IsValid()) {
    content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
    for (auto it = urls.begin(); it != urls.end(); ++it) {
      if (rfh) {
        rfh->GetAllImage(taskid, *it,
                         base::BindOnce(&ArkwebFrameHostExtImpl::OnGetAllImage,
                         static_cast<ArkwebFrameHostExtImpl*>(frame.get()),
                         taskid
                         ));
      }
...
void ArkWebBrowserHostExtImpl::GetAllImageByXPath(int32_t taskid, const std::vector<std::string>& xpath) {
  LOG(INFO) << "ArkWebBrowserHostExtImpl::GetAllImageByXPath";
  auto web_contents = GetWebContents();
  auto frame = GetMainFrame();
  if (web_contents && frame && frame->IsValid()) {
    content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
    for (auto it = xpath.begin(); it != xpath.end(); ++it) {
      if (rfh) {
        rfh->GetImageByXPath(taskid, *it,
                  base::BindOnce(&ArkwebFrameHostExtImpl::OnGetImageByXPath,
                  static_cast<ArkwebFrameHostExtImpl*>(frame.get()),
                  taskid
                  ));
      }
...
#if BUILDFLAG(IS_ARKWEB_EXT)
    ohos_adblock::AdblockConfigBridge::GetInstance()->EnableAdsBlock(true);
  } else {
    ohos_adblock::AdblockConfigBridge::GetInstance()->EnableAdsBlock(false);
  }
#else
    ohos_adblock::AdblockConfigBridge::GetInstance()->EnableAdsBlock(GetBrowserContext(), true);
  } else {
    ohos_adblock::AdblockConfigBridge::GetInstance()->EnableAdsBlock(GetBrowserContext(), false);
  }
#endif
}
...
#if BUILDFLAG(IS_ARKWEB_EXT)
      ->EnableIntelligentTrackingPrevention(enable);
#else
...
void ArkWebBrowserHostExtImpl::OnSafeBrowsingDetectionResult(
    const SafeBrowsingDetectionResult& safeBrowsingDetectionResult) {
  LOG(INFO) << "code is " << safeBrowsingDetectionResult.code
            << ",policy is " << safeBrowsingDetectionResult.policy
            << ",mapping type is " << safeBrowsingDetectionResult.mappingType ;

  if (!CEF_CURRENTLY_ON_UIT()) {
    CEF_POST_TASK(CEF_UIT, base::BindOnce(
                               &ArkWebBrowserHostExtImpl::OnSafeBrowsingDetectionResult,
                               this, safeBrowsingDetectionResult));
    return;
  }
  auto web_contents = GetWebContents();
  if (!web_contents) {
  ...
  if(safeBrowsingDetectionResult.isContentCheck){
    ohos_safe_browsing::SafeBrowsingSnapshotManager::GetInstance().
        SetNewUrl(safeBrowsingDetectionResult.accessUrl,
        safeBrowsingDetectionResult, web_contents->GetNWebId());
    LOG(INFO) << "safeBrowsingDetectionResult isContentCheck is true";
    return;
  }
  
  ohos_safe_browsing::SafeBrowsingTabHelper* safe_browsing_tab_helper =
      ohos_safe_browsing::SafeBrowsingTabHelper::FromWebContents(web_contents);
#if BUILDFLAG(ARKWEB_EX_FALLBACK_PROXY)
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
          ::switches::kEnableNwebEx)) {
    fallback_proxy::FallbackProxyService::GetInstance()
        ->SaveURLMaliciousTypeAndHwCode(safeBrowsingDetectionResult.accessUrl, 
        safeBrowsingDetectionResult.policy, safeBrowsingDetectionResult.code);
  }
#endif  // ARKWEB_EX_FALLBACK_PROXY
  if (!safe_browsing_tab_helper) {
    LOG(ERROR) << "SafeBrowsingTabHelper has no safe_browsing_tab_helper.";
    return;
  }

  safe_browsing_tab_helper->OnDetectionResult(safeBrowsingDetectionResult.code, 
                                              safeBrowsingDetectionResult.policy,
                                              safeBrowsingDetectionResult.mappingType, 
                                              safeBrowsingDetectionResult.accessUrl);
}
...
const blink::UserAgentMetadata ArkWebBrowserHostExtImpl::GetUserAgentMetadata(
  ...
  if (web_contents == nullptr) {
    LOG(DEBUG) << kUserAgentMetadataTag << " web_contents is nullptr, nweb_id "
               << GetNWebId();
    return AlloyBrowserUAConfig::GetInstance()->GetDefaultUserAgentMetadata();
  }
...
#if BUILDFLAG(ARKWEB_READER_MODE)
void ArkWebBrowserHostExtImpl::Distill(uint64_t request_id, const DistillOptions& distill_options,
  ...
  oh_dom_distiller::DistillResultCallback distill_result_callback =
    base::BindOnce(
      [](CefRefPtr<CefDistillCallback> callback,
        uint64_t request_id, const std::string& content) {
        if (callback) {
          callback->OnDistillCallback(request_id, content);
        }
      },
      callback, request_id);
