...
OhGinJavascriptBridgeDispatcher::OhGinJavascriptBridgeDispatcher(
    ...
    : content::RenderFrameObserver(render_frame),
      ...
      converter_(new OhGinJavascriptBridgeValueConverter(AsWeakPtr())) {}
...
OhGinJavascriptBridgeDispatcher::H5ObjectID OhGinJavascriptBridgeDispatcher::AddH5Object(
    v8::Local<v8::Object>& value) {
  LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::AddH5Object called";
  v8::Isolate* isolate = blink::MainThreadIsolate();
  v8::HandleScope handle_scope(isolate);

  v8::Persistent<v8::Value>* persistent = new v8::Persistent<v8::Value>(isolate, value);

  H5ObjectID object_id;
  {
...
std::vector<std::string> OhGinJavascriptBridgeDispatcher::GetH5ObjectMethodNames(
  ...
  if(!object->GetPropertyNames(context,
  ...
  LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::GetH5ObjectMethodNames called";
  for (uint32_t i = 0; i < keys->Length(); ++i) {
    ...
    v8::MaybeLocal<v8::Value> maybe_value =
        object->Get(context, key);
    if (try_catch.HasCaught() || !maybe_value.ToLocal(&value)) {
      LOG(WARNING) << "OhGinJavascriptBridgeDispatcher::GetH5ObjectMethodNames Getter property fail";
      continue;
    }
...
bool OhGinJavascriptBridgeDispatcher::HasH5ObjectMethod(
  ...
  if(!object->GetOwnPropertyNames(context).ToLocal(&keys)) {
    LOG(ERROR) << "OhGinJavascriptBridgeDispatcher::HasH5ObjectMethod GetOwnPropertyNames fail";
    return false;
  }
  ...
  for (uint32_t i = 0; i < keys->Length(); ++i) {
    ...
    v8::MaybeLocal<v8::Value> maybe_value =
        object->Get(context, key);
    if (try_catch.HasCaught() || !maybe_value.ToLocal(&value)) {
      LOG(WARNING) << "OhGinJavascriptBridgeDispatcher::HasH5ObjectMethod Getter for property "
...
void OhGinJavascriptBridgeDispatcher::OnDoCallAnonymousH5Function(
  ...
  std::map<H5ObjectID, v8::Persistent<v8::Value>*>::iterator
      iter;
  {
    base::AutoLock locker(h5_objects_lock_);
    iter = h5_object_map_.find(h5_object_id);
    if (iter == h5_object_map_.end() || !(iter->second)) {
      LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallAnonymousH5Function obj id not found";
      return;
    }
  }
  ...
  if (!render_frame() || !render_frame()->GetWebFrame()) {
    LOG(ERROR) << "OhGinJavascriptBridgeDispatcher::OnDoCallAnonymousH5Function frame null";
    return;
  }
  ...
  if(context.IsEmpty()) {
    LOG(ERROR) << "OhGinJavascriptBridgeDispatcher::OnDoCallAnonymousH5Function context empty";
    return;
  }
  v8::Context::Scope context_scope(context);
  ...
  int size = args.GetList().size();
  if (size > 0) {
    LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallAnonymousH5Function args size = " << size;
    v8_args = new v8::Local<v8::Value>[size];
  }

  for (int i = 0; i < size; ++i) {
    const base::Value& base_arg =
        args.GetList()[i];
    if (!base_arg.is_blob()) {
      LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallAnonymousH5Function not blob";
    ...
    if (gin_value->IsType(OhGinJavascriptBridgeValue::TYPE_OBJECT_ID)) {
      ...
      if (gin_value->GetAsObjectID(&object_id)) {
        LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallAnonymousH5Function blob";
        object_result = GetObject(object_id);
      }
      if(object_result) {
        LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallAnonymousH5Function object_result ok";
        gin::Handle<OhGinJavascriptBridgeObject> controller =
            gin::CreateHandle(isolate, object_result);
        if(controller.IsEmpty()) {
          LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallAnonymousH5Function controller empty";
  ...
  LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallAnonymousH5Function func call end";
...
void OhGinJavascriptBridgeDispatcher::convertToV8Value(const base::ListValue& base_value, v8::Local<v8::Value>*& v8_value) {
  v8::Isolate* isolate = blink::MainThreadIsolate();
  v8::HandleScope handle_scope(isolate);
  ...
  int size = base_value.GetList().size();
  for (int i = 0; i < size; ++i) {
    const base::Value& base_arg =
        base_value.GetList()[i];
    if (!base_arg.is_blob()) {
...
void OhGinJavascriptBridgeDispatcher::OnDoCallH5Function(
  ...
  if (!render_frame() || !render_frame()->GetWebFrame()) {
    LOG(ERROR) << "OhGinJavascriptBridgeDispatcher::OnDoCallH5Function frame null";
    return;
  }
  ...
  if(context.IsEmpty()) {
    LOG(ERROR) << "OhGinJavascriptBridgeDispatcher::OnDoCallH5Function context empty";
    return;
  }
  v8::Context::Scope context_scope(context);
  ...
  for (int i = 0; i < size; ++i) {
    const base::Value& base_arg =
        args.GetList()[i];
    if (!base_arg.is_blob()) {
      LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallH5Function not blob";
      v8_args[i] = converter_->ToV8Value(&base_arg, context);
      continue;
    }
    ...
    if (gin_value->IsType(OhGinJavascriptBridgeValue::TYPE_OBJECT_ID)) {
      ...
      OhGinJavascriptBridgeDispatcher::ObjectID object_id;
      if (gin_value->GetAsObjectID(&object_id)) {
        LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallH5Function blob";
        object_result = GetObject(object_id);
      }
      if(object_result) {
        LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallH5Function object_result ok";
        gin::Handle<OhGinJavascriptBridgeObject> controller =
            gin::CreateHandle(isolate, object_result);
        if(controller.IsEmpty()) {
          LOG(DEBUG) << "OhGinJavascriptBridgeDispatcher::OnDoCallH5Function controller empty";
          v8_args[i] = v8::Undefined(isolate);
          continue;
        }
        v8_args[i] = controller.ToV8();
  ...
  v8::TryCatch try_catch(isolate);
  v8::MaybeLocal<v8::Value> result_value =
      object->Get(context, key);
