...
// short enough to avoid obvious fill latency. 100ms is an experience-based
// value and may need tuning with further validation.
...
void OhAutofillClient::OnFocusChangedInPage(content::FocusedNodeDetails* details) {
  if (!details) {
    return;
  }
