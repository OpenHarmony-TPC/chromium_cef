...
void ArkwebFrameHostExtImpl::OnGetAllImage(
  int32_t taskid,
  uint32_t buffer_size,
  base::ReadOnlySharedMemoryRegion region) {
  LOG(INFO) << "ohmedia: OnGetAllImage start task";
  base::ThreadPool::PostTask(
    FROM_HERE,
    {base::MayBlock(), base::TaskPriority::USER_VISIBLE},
    base::BindOnce(
      &ArkwebFrameHostExtImpl::OnGetAllImageTask,
...
void ArkwebFrameHostExtImpl::OnGetAllImageTask(
    int32_t taskid,
    uint32_t buffer_size,
    base::ReadOnlySharedMemoryRegion region) {
  auto browser = GetBrowserHostBase();
  ...
  if (!region.IsValid()) {
    LOG(ERROR)
        << "OnGetAllImage: Read-only shared memory region is invalid";
    handler->AsCefContextMenuHandlerExt()->OnGetAllImage(taskid, nullptr, 0);
    return;
  }
  base::ReadOnlySharedMemoryMapping mapping = region.MapAt(0, buffer_size);
  if (!mapping.IsValid()) {
    LOG(ERROR)
        << "OnGetAllImage: Read-only shared memory mapping is invalid";
    handler->AsCefContextMenuHandlerExt()->OnGetAllImage(taskid, nullptr, 0);
    return;
  }

  uint8_t* buffer = (uint8_t*)(mapping.memory());
  handler->AsCefContextMenuHandlerExt()->OnGetAllImage(taskid, buffer, buffer_size);
}

void ArkwebFrameHostExtImpl::OnGetImageByXPath(
    int32_t taskid,
    uint32_t buffer_size,
    base::ReadOnlySharedMemoryRegion region,
    uint32_t width,
    uint32_t height) {
  LOG(INFO) << "ArkwebFrameHostExtImpl::OnGetImageByXPath";
  auto browser = GetBrowserHostBase();
  if (!browser) {
    return;
  }
  ...
  int32_t image_id = handler->AsCefContextMenuHandlerExt()->GetImageId(taskid);
  base::ThreadPool::PostTask(
    FROM_HERE,
    {base::MayBlock(), base::TaskPriority::USER_VISIBLE},
    base::BindOnce(
      &ArkwebFrameHostExtImpl::OnGetImageByXPathTask,
      weak_ptr_factory_.GetWeakPtr(),
      taskid,
      buffer_size,
      std::move(region),
...
void ArkwebFrameHostExtImpl::OnGetImageByXPathTask(
    int32_t taskid,
    uint32_t buffer_size,
    base::ReadOnlySharedMemoryRegion region,
    uint32_t width,
    ...
    CefImageImpl* image_impl = new (std::nothrow) CefImageImpl();
    if (!image_impl) {
      LOG(ERROR) << "OnGetImageByXPath: Failed to allocate CefImageImpl";
      handler->AsCefContextMenuHandlerExt()->OnGetImageByXPath(taskid, nullptr, image_id);
      return;
    }
    if (!region.IsValid()) {
      LOG(ERROR)
          << "OnGetImageByXPath: Read-only shared memory region is invalid";
      handler->AsCefContextMenuHandlerExt()->OnGetImageByXPath(taskid, image_impl, image_id);
      return;
    }
    base::ReadOnlySharedMemoryMapping mapping = region.MapAt(0, buffer_size);
    if (!mapping.IsValid()) {
      LOG(ERROR)
          << "OnGetImageByXPath: Read-only shared memory mapping is invalid";
      handler->AsCefContextMenuHandlerExt()->OnGetImageByXPath(taskid, image_impl, image_id);
      return;
    }
    ...
    SkBitmap bitmap;
    bool installed = bitmap.installPixels(
      info,
      pixels,
      info.minRowBytes()
    );
