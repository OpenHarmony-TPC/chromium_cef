...
// Helper function to convert PermissionDescriptorPtr to PermissionType
std::vector<blink::PermissionType> ConvertPermissionDescriptors(
    const std::vector<blink::mojom::PermissionDescriptorPtr>& descriptors) {
  std::vector<blink::PermissionType> permissions;
  permissions.reserve(descriptors.size());
  for (const auto& descriptor : descriptors) {
    permissions.push_back(static_cast<blink::PermissionType>(descriptor->name));
  }
  return permissions;
}

// Helper function to convert PermissionName to PermissionType
blink::PermissionType ConvertPermissionNameToType(
    blink::mojom::PermissionName name) {
  return static_cast<blink::PermissionType>(name);
}
 ...
 public:
  UnhandledRequest(const std::vector<PermissionType> permissions,
      ...
      : permissions_(permissions),
        ...
        results_(permissions.size(), content::PermissionResult(blink::mojom::PermissionStatus::DENIED, content::PermissionStatusSource::UNSPECIFIED)),
        cancelled_(false) {
  ...
  void SetPermissionStatus(PermissionType type, PermissionStatus status) {
    ...
    DCHECK(!IsCompleted());
    results_[result->second] = content::PermissionResult(status, content::PermissionStatusSource::UNSPECIFIED);
  ...
  PermissionStatus GetPermissionStatus(PermissionType type) {
    ...
    return results_[result->second].status;
  }
...
void AlloyPermissionManager::RequestPermissions(
  ...
  const std::vector<blink::PermissionType> permissions =
      ConvertPermissionDescriptors(request_description.permissions);
  ...
  if (pending_request_raw->IsCompleted()) {
    std::vector<content::PermissionResult> results = pending_request_raw->results_;
...
void AlloyPermissionManager::OnRequestResponseCallBack(
  ...
  std::vector<
      std::pair<RequestPermissionsCallback, std::vector<content::PermissionResult>>>
...
PermissionStatus AlloyPermissionManager::GetPermissionStatus(
  ...
  blink::PermissionType permission_type = ConvertPermissionNameToType(permission->name);
  if (permission_type == PermissionType::CLIPBOARD_READ_WRITE ||
      permission_type == PermissionType::CLIPBOARD_SANITIZED_WRITE) {
    return PermissionStatus::GRANTED;
#if BUILDFLAG(ARKWEB_SENSOR)
  } else if (permission_type == PermissionType::SENSORS) {
    return PermissionStatus::ASK;
#endif  //BUILDFLAG(ARKWEB_SENSOR)
#if BUILDFLAG(ARKWEB_ARKWEB_EXTENSIONS)
  } else if (permission_type == PermissionType::NOTIFICATIONS) {
    if ((*base::CommandLine::ForCurrentProcess()).HasSwitch(
  ...
  } else if (permission_type == PermissionType::GEOLOCATION) {
    if ((*base::CommandLine::ForCurrentProcess()).HasSwitch(
        switches::kEnableNwebExPermission)) {
...
#endif // #if BUILDFLAG(ARKWEB_ARKWEB_EXTENSIONS)
#if BUILDFLAG(ARKWEB_WEBRTC)
  } else if (permission_type == PermissionType::AUDIO_CAPTURE) {
    if ((*base::CommandLine::ForCurrentProcess()).HasSwitch(
        switches::kEnableNwebExPermission)) {
  ...
  } else if (permission_type == PermissionType::VIDEO_CAPTURE) {
    if ((*base::CommandLine::ForCurrentProcess()).HasSwitch(
        switches::kEnableNwebExPermission)) {
...
AlloyPermissionManager::GetPermissionResultForOriginWithoutContext(
  ...
  blink::mojom::PermissionStatus status = GetPermissionStatus(
      std::move(permission), requesting_origin.GetURL(), requesting_origin.GetURL());
...
content::PermissionResult AlloyPermissionManager::GetPermissionResultForCurrentDocument(
    const blink::mojom::PermissionDescriptorPtr& permission_descriptor,
    content::RenderFrameHost* render_frame_host,
    bool should_include_device_status) {
  // TODO(arkweb): Chromium 141 compatibility - implement proper permission checking
  // This function needs to be implemented based on ArkWeb's permission system
  LOG(INFO) << "GetPermissionResultForCurrentDocument called with default implementation";

  // Return default permission result - ASK is safe default that prompts user interaction
  using PermissionStatus = blink::mojom::PermissionStatus;
  PermissionStatus status = PermissionStatus::ASK;
  content::PermissionStatusSource source = content::PermissionStatusSource::UNSPECIFIED;
